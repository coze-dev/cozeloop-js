// Copyright (c) 2026 Bytedance Ltd. and/or its affiliates
// SPDX-License-Identifier: MIT
import { setTimeout } from 'node:timers/promises';

import { config } from 'dotenv';

import { setupLoopTracerMock } from '../__mock__/loop-tracer';
import { simpleConsoleLogger } from '../../src/utils/logger';
import { cozeLoopTracer, SpanKind } from '../../src/tracer';
import {
  type LoopTraceLLMCallOutput,
  type LoopTraceLLMCallInput,
} from '../../src/api';

config();

cozeLoopTracer.initialize({
  apiClient: {
    logger: simpleConsoleLogger,
  },
  ultraLargeReport: true,
});

describe('Test Trace Report', { timeout: 50_000 }, () => {
  const httpMock = setupLoopTracerMock();
  beforeAll(async () => {
    httpMock.start();
    await setTimeout(1000, 1);
  });
  afterAll(async () => {
    await setTimeout(1000, 1);
    httpMock.close();
  });
  afterEach(() => httpMock.reset());

  it('#1 Test report with file upload', async () => {
    const generateLargeString = (sizeInMB: number) => {
      const repeats = sizeInMB * 1024 * 1024;
      return 'x'.repeat(repeats);
    };

    await cozeLoopTracer.traceable(
      async span => {
        span.setAttribute('custom-tag-1', 'custom-value-1');

        const mockParentInput: LoopTraceLLMCallInput = {
          messages: [
            {
              role: 'user',
              content: generateLargeString(5),
              name: 'test-name',
            },
          ],
          tools: [
            {
              type: 'function',
              function: {
                name: 'test-tool',
                description: 'tool-description',
                parameters: {
                  type: 'object',
                  properties: {
                    name: { type: 'string' },
                  },
                },
              },
            },
          ],
          tool_choice: {
            type: 'auto',
            function: {
              name: 'test-tool',
            },
          },
        };
        const mockParentOutput: LoopTraceLLMCallOutput = {
          choices: [
            {
              index: 0,
              finish_reason: 'stop',
              message: {
                role: 'assistant',
                content: 'test-content',
              },
            },
          ],
        };

        cozeLoopTracer.setInput(span, mockParentInput);
        cozeLoopTracer.setOutput(span, mockParentOutput);

        await cozeLoopTracer.traceable(
          childSpan => {
            const mockChildInput: LoopTraceLLMCallInput = {
              messages: [
                {
                  role: 'user',
                  content: 'child-content',
                  name: 'test-name',
                  parts: [
                    {
                      type: 'image_url',
                      image_url: {
                        url: 'data:image/png;base64,',
                      },
                    },
                    {
                      type: 'file_url',
                      file_url: {
                        name: 'test.txt',
                        url: 'data:text/plain;base64,dGVzdCB0ZXN0IHRlc3QK',
                      },
                    },
                  ],
                },
              ],
            };

            cozeLoopTracer.setInput(childSpan, mockChildInput);

            return 'test-result2';
          },
          {
            name: 'TestChild',
            type: SpanKind.Model,
          },
        );

        return 'test-result1';
      },
      {
        name: 'TestParent',
        type: SpanKind.PromptTemplate,
        baggages: {
          user_id: 'test-user-id',
        },
        attributes: {
          string_tag: 'string',
          boolean_tag: true,
          number_tag: 123,
          float_tag: 1.23,
          [`long_text_key_tag_${generateLargeString(1)}`]: 'string',
          long_text_value_tag: generateLargeString(1),
        },
      },
    );
  });

  it('#2 Test traceable with error', async () => {
    try {
      await cozeLoopTracer.traceable(
        () => {
          throw new Error('test-error');
        },
        {
          name: 'TestTraceableWithError',
          type: SpanKind.PromptTemplate,
        },
      );
    } catch (error) {
      expect(error).toBeInstanceOf(Error);
    }
  });

  it('#3 Test force flush report', async () => {
    await cozeLoopTracer.traceable(() => 'test-force-flush', {
      name: 'TestForceFlushReport',
      type: SpanKind.PromptTemplate,
    });

    cozeLoopTracer.forceFlush();
  });

  it('#4 Test shutdown report', async () => {
    await cozeLoopTracer.traceable(
      async () => {
        await cozeLoopTracer.traceable(() => 'test-shutdown-child', {
          name: 'TestShutdownReportChild',
          type: SpanKind.Model,
        });

        cozeLoopTracer.shutdown();

        return 'test-shutdown-parent';
      },
      {
        name: 'TestShutdownReportParent',
        type: SpanKind.PromptTemplate,
      },
    );
  });
});
